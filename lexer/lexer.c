...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/03 01:18:19 by mmoutawa          #+#    #+#             */
/*   Updated: 2022/09/03 01:18:20 by mmoutawa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*get_next_token(t_lexer *lexer, t_env_list *env)
{
	while (lexer->c != '\0')
	{
		if (lexer->c == ' ')
			...
			return (advance_token(lexer,
					init_token(TOKEN_PIPE, get_char_as_string(lexer))));
		else if (lexer->c == '<' || lexer->c == '>')
